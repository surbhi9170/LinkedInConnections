...
function injectTheScript() {
  ...
  chrome.scripting.executeScript({ target: { tabId: tabs[0].id },  files: ["content-script-new.js"] });
  });
}
...
testButton.addEventListener("click", () => {
  ...
  if(flag==0)
  {
    ...
    console.log("id :::::::",p);
    chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
      chrome.scripting.executeScript({ target: { tabId: tabs[0].id },  files: ["reload.js"] });
      });

    // p.click()
    // document.getElementById('ember64').click();
  }
